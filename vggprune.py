...
parser.add_argument('--save', default='logs_prune', type=str, metavar='PATH',
...
def main():
    ...
    pruned = 0
    cfg = []  # [49, 64, 'M', 127, 127, 'M', 255, 245, 213, 160, 'M', 121, 45, 4, 3, 'M', 1, 5, 86, 146, 'M']
    # [], every element in this list is a mask of a tensor, for example: torch.array([True, False, True, ....])
    cfg_mask = []

    # Here is to be finised, calculate cfg and cfg_mask
    ...
    for k, m in enumerate(model.modules()):
        if isinstance(m, nn.BatchNorm2d):
            ...
            # if(max(mask)==0):
            #     mask[weight_copy.max(-1)[1]] = 1
            if mask.max(-1)[1].item() == 0.:
                mask[weight_copy.max(-1)[1]] = 1
                mask[0:9] = 1  
            # 此处和王天洪同学讨论过
